...
class App:
    def __init__(self, root):
        ...
        self.menu = self.data.json_data
        self.images = self.data.image_data
        self.small_images = self.data.small_image_data
        self.order = []
        self.total_price = 0
        ...
        master_state = states.Master(
            root, self.small_images[len(self.small_images)-1]['img_file'])
    ...
    def state_manager(self):
        ...
        self.start_state = states.Start_State(
            self.state_frame, [self.state_picker, self.clear_order])
        self.menu_state = states.Menu_State(self.state_frame, self.unique_categories, self.menu,
                                            self.images, self.small_images, self.order, [self.state_picker, self.update_order])
        self.checkout_state = states.Checkout_State(self.state_frame, self.order, self.small_images, [
                                                    self.checkout_payment, self.update_order, self.reliable_round])
        self.popup = states.PopUp(
            self.restart_timer, self.reliable_round, self.restart)
    ...
    def state_picker(self, target):
        ...
        if target == 0:
            ...
            self.navigation_buttons[0].config(
                text='Food\nMenu', command=lambda: self.state_picker(1))
            self.navigation_buttons[1].config(
    ...
    def restart_timer(self, root, order_num, payment_type, change):
        self.root.after(10000, lambda: self.restart(
            root, order_num, payment_type, change))

    def restart(self, root, order_num, payment_type, change, instant=False):
        if instant == True:
            self.store_order(order_num=order_num,
                             payment=payment_type, change=change)
            root.destroy()
            self.clear_order()
            self.state_picker(0)
            self.state_manager()
    ...
    def store_order(self, order_num, payment, change):
        self.data.write_orders(
            [order_num, self.total_price, payment, self.order, change])
